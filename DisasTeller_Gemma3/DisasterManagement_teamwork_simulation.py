...
Gemma_3 = ChatOllama(model="gemma3:27b", temperature=0.8, top_p=0.8)
...
Expert_team = Agent(
    role="Expert team",
    goal="Analyse the contents got from the detailed observations from the local images, use **offline_pdf_search_tool** and **global_map_annotation**",
    ...
    llm=Gemma_3,
    tools=[local_img_interpreter, global_map_annotation, offline_pdf_search_tool]
)
...
agents = [Expert_team, Alerts_team, Emergency_team, Assign_team]

task1 = Task(
    description="""
    1. Use **local_img_interpreter** with text input:"text": "get disaster description for all locations.".
    2. You must search 'offline_pdf_search_tool', get a concise summary of the disaster region from step 1 with their 
    relevant grading, finally output this format: "text": "location:xxx, grading:xxx". This step is mandatory.
    3. send concise summary of location name and disaster grading to **global_map_annotation**. This step is mandatory.
    4. Generate a final 2000-word report with additional description from step 1.
    """,
    tools=[offline_pdf_search_tool, global_map_annotation, local_img_interpreter],
    agent=Expert_team,
    expected_output=""" 
    1. Use **local_img_interpreter** to get "location name: xxx, description: xxx"
    2. call **pdf_search_tool** to get "location name:xxx, grading:xxx"
